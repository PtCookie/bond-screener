...
- **D1 접근**: `src/lib/d1/sql.ts`가 `json_each(?1)`로 파라미터 1개에 배열을 실어 쿼리 1개로 벌크 upsert하는 SQL을 생성한다(Worker의 "bound parameter 100개/쿼리" 제한 우회 — invocation당 쿼리 수 한도는 Paid에서 1000개로 늘었지만 쿼리당 bound parameter 100개는 플랜과 무관하게 그대로라 이 기법 자체는 계속 필요하다). **`INSERT ... SELECT ... FROM json_each(?1) ON CONFLICT ...` 형태는 SQLite 파서가 "near 'DO': syntax error"를 내므로 반드시 `FROM` 절과 `ON CONFLICT` 사이에 `WHERE true` 더미절을 넣어야 한다** — `buildInsertSelect` 헬퍼가 이미 방어하고 있으니 새 upsert SQL을 추가할 때도 이 헬퍼를 거칠 것.
- **cron 파이프라인**: `src/worker.ts`의 `scheduled` → `src/lib/sync/tick.ts`(`runSyncTick`). 한 tick(1분 간격) 안에서 `TICK_WALL_BUDGET_MS`(20초)·`MAX_PAGES_PER_TICK`(40) 중 먼저 닿는 예산까지 페이지를 이어 처리한다(`src/lib/sync/config.ts`) — Paid의 cron CPU 한도(30초)가 페이지당 실측 CPU(~12ms, `ISSU_PAGE_SIZE=1000`)에 비해 넉넉해, 실질 상한은 오픈API 응답 지연(wall-clock)이다. `runPagesUntilBudget`은 매 페이지 처리 후 D1에서 커서를 다시 읽어 진행 여부를 판단한다 — step이 반환하는 `done:false`만으로는 "정상적으로 다음 페이지로 넘어갈 준비가 됐다"와 "backoff로 이번 페이지를 아예 못 넘겼다"(오픈API rate limit, 커서 불변)를 구분할 수 없어서다. 커서가 안 움직였으면 즉시 멈추고 다음 tick에 맡긴다 — 안 그러면 같은 rate-limited 페이지를 예산 소진까지 헛되이 재시도한다. **기본정보(`issu`)는 2026-09부터 시세와 동일하게 매 영업일 수집한다**(예전엔 주 1회 지정 요일이었다 — `src/lib/sync/plan.ts`의 요일 게이트를 제거했다). 오늘 대상 basDt로 `issu`가 `done`이 되면, **다음 tick**에서 스크리너 스냅샷 파이프라인이 자동으로 이어진다(아래 "스크리너 스냅샷" 절) — base 전량 재빌드 요일(`SNAPSHOT_REBUILD_WEEKDAY_KST`, 기본 수요일)이면 `snapshot` 액션, 그 외 평일엔 그날 변경분만 담는 `bondDelta` 액션이다. `planTick`이 `app_meta`의 `snapshot_bas_dt`/`bond_delta_bas_dt`를 그 basDt와 비교해 판단하며, 기본정보용 `sync_run`과 달리 이 둘은 전용 `sync_run` 행을 만들지 않는다(멱등한 재시도라 좀비 run 위험이 없다 — `getRunningSyncRun`이 source 무관으로 아무 `running` 행이나 잡아 이후 모든 tick을 묶는 구조라, 만에 하나 스냅샷/델타 빌드가 catch 밖에서 죽으면 파이프라인 전체가 막힐 수 있기 때문).
- **`sync_run.status`는 `done`과 `empty`를 구분한다.** 오픈API 갱신이 영업일+1일 오후 1시 이후라 cron이 먼저 조회하면 0건이 온다 — 이걸 `done`으로 마감하면 다음 tick이 그 basDt를 영영 재조회하지 않는다(실운영에서 실제 발생). `finishSyncRun`은 `totalCount===0`이면 `empty`로 마감하고, `planTick`은 `empty`를 미시작(`null`)과 동일하게 재시도 대상으로 본다. **단 즉시 재시도하지는 않는다** — `EMPTY_RETRY_BACKOFF_MS`(15분, `src/lib/sync/config.ts`)가 지나야 재시작 대상으로 본다. 백오프가 없으면 cron이 매분(`*/1 4-8 * * *`) 같은 미발행 basDt를 헛되이 재조회한다(원격 `sync_run`에 `price attempt=601` 실측 — 주말 내내 empty였던 basDt를 5시간 창 동안 매분 재시도한 흔적이며, 기본정보가 매일 도는 지금은 시세·기본정보 양쪽에서 이 문제가 배로 생길 수 있어 함께 도입했다). `startSyncRun`은 새 컬럼을 추가하지 않고 기존 `finished_at`(없으면 `updated_at`)만으로 경과 시간을 판단한다. **`startSyncRun`은 `empty`/`failed` 상태에서 재시작될 때 `next_page`/`total_count`/`rows_seen`/`rows_written`/`finished_at`/`error`를 전부 리셋한다** — 리셋하지 않으면 이전 시도의 `next_page`가 남아 `tick.ts`가 메모리에서 합성하는 `next_page: 1`과 어긋나고, 이후 `resume` 경로가 DB 값을 그대로 읽으면서 앞쪽 페이지를 영구히 건너뛴다(과거 실제로 있었던 버그, `tests/workers/sync-run-repo.test.ts`/`tick.test.ts`로 회귀 방지 중).
...
- **테스트**: vitest는 `test.projects`로 **세 프로젝트**를 돈다(`vitest.config.ts`가 오케스트레이터, 실제 설정은 `vitest.node.config.ts`/`vitest.browser.config.ts`/`vitest.workers.config.ts`). 바인딩(D1/R2)이 필요 없는 순수 로직은 **node 프로젝트**(`tests/*.test.ts`, `tests/scripts/**`, astro의 `getViteConfig` 사용)에, DOM이 필요한 React 컴포넌트·훅 테스트는 **browser 프로젝트**(`tests/components/**`/`tests/hooks/**`, Vitest Browser Mode — 아래 별도 항목)에, D1/R2 바인딩이 필요한 테스트는 **workers 프로젝트**(`tests/workers/**`, `@cloudflare/vitest-plugin`의 `cloudflareTest()`로 실제 workerd 런타임 위에서 실행)에 둔다. `astro.config.mjs`의 `isVitest` 분기(위 "알려진 이슈" 참고) 때문에 astro 어댑터 경유로는 Cloudflare 런타임을 못 쓰지만, `@cloudflare/vitest-plugin`은 이 충돌과 무관하게 독립적으로 workerd를 띄우므로 실제 D1/R2 바인딩 테스트가 가능하다. 패키지명이 `@cloudflare/vitest-pool-workers`에서 `@cloudflare/vitest-plugin`으로 개명됐고(v1, 2026-08-19), `defineWorkersConfig`/`defineWorkersProject`도 제거돼 `cloudflareTest()` Vite 플러그인을 일반 `defineConfig`에 넣는 방식으로 바뀌었다 — 옛 예제·문서를 따라 하지 말 것. `readD1Migrations`/`cloudflareTest`는 설치된 버전(1.1.x) 기준 `@cloudflare/vitest-plugin` 메인 엔트리에서 바로 export되며 `/config` 서브패스는 없다(공식 문서가 이 서브패스를 언급하지만 실제 `package.json`의 `exports`에는 없음 — 실측 확인). `@cloudflare/vitest-plugin`의 스토리지 격리는 **테스트 파일 단위**라 같은 파일의 여러 `test()`는 상태를 공유한다 — `test()` 단위 격리가 필요하면 `tests/workers/helpers/reset-d1.ts`의 `resetD1()`(`cloudflare:test`의 `reset()` + `applyD1Migrations` 재적용)을 `beforeEach`에 둘 것. `tests/helpers/fake-d1.ts`(`node:sqlite` 기반)는 이제 legacy다 — `tests/d1-integration.test.ts`가 아직 이걸 쓰고 있어 당장 지우지는 않지만, 새 D1 테스트는 workers 프로젝트 쪽에 실제 바인딩으로 작성할 것. `tests/workers/**`에서 `vi.spyOn(env.DB, ...)`/`vi.spyOn(env.ARCHIVE, ...)`로 바인딩을 고장내는 테스트를 쓸 때: (1) `startSyncRun` 같은 준비 호출이 끝난 **뒤에** 스파이를 걸 것 — 먼저 걸면 준비 단계 자체의 D1 호출까지 스텁 카운트를 먹어 의도한 호출에서 안 터진다. (2) `afterEach`에 `vi.restoreAllMocks()`를 둘 것 — 테스트 안의 개별 `mockRestore()`는 그 앞 assert가 실패하면 실행되지 않아 스파이가 다음 테스트로 샌다.
  - **`vitest.node.config.ts`의 `exclude`에서 `e2e/**`를 빼면 안 된다.** node 프로젝트의 기본 include(`**/*.{test,spec}.*`)가 제외 없이는 `e2e/*.spec.ts`(Playwright 전용 `test()`)까지 수집해 `Error: Playwright Test did not expect test() to be called here`로 전체가 실패한다 — 실제로 겪은 문제(Playwright 도입 시 `e2e/`가 새로 생기면서 발생).
  - **Vitest Browser Mode(`vitest.browser.config.ts`, `tests/components/**`/`tests/hooks/**`)**: jsdom이 아니라 실제 브라우저(Playwright provider, chromium/firefox/webkit)에서 돈다. jsdom 대신 이걸 쓰는 이유: `@base-ui/react` 팝오버가 `Element.getAnimations()`/`ResizeObserver`/pointer capture를 요구해 jsdom에선 폴리필해도 불안정하고, `PriceChart`(lightweight-charts)가 canvas 2D context를 요구하며, `useIsMobile`이 `matchMedia`를 실제로 평가해야 한다. `vitest.node.config.ts`와 동일하게 astro의 `getViteConfig()`를 경유해 `@/` 별칭·Tailwind가 그대로 따라온다. `vitest-browser-react`의 `render`/`renderHook`은 **async**(await 필수, 빠뜨리면 `screen.getByText is not a function` 등 알기 어려운 에러) — 자동 cleanup은 메인 엔트리(`vitest-browser-react`, `/pure` 아님)가 제공하지만, `useScreenerViewState`처럼 `window.history`/`sessionStorage`를 직접 건드리는 훅은 자동 cleanup 대상이 아니라 `tests/setup-browser.ts`의 `afterEach`가 매 테스트 후 원래 URL로 `replaceState`하고 `sessionStorage.clear()`한다(안 하면 훅 테스트끼리 서로의 URL·스토리지를 오염시킨다). Base UI 팝오버는 Portal로 body 직속에 렌더된다 — 필터 팝오버 텍스트가 표 셀(등급 Badge 등)과 겹쳐 전역 쿼리가 모호해지면 `document.querySelector('[data-slot="popover-content"]')`로 스코프를 좁힐 것(Vitest: `page.elementLocator(...)`, Playwright: `page.locator(...)`). 컴포넌트 테스트에서 `screenerFeatures`로 직접 `useTable` 하네스를 만들 때 `autoResetPageIndex: false`를 빠뜨리면 TanStack Table이 매 상태 변경마다 pageIndex를 0으로 되돌려 페이지 이동 자체가 무력화된다(BondScreener.tsx가 프로덕션에서 이미 이 옵션을 켜는 이유와 동일) — 테스트 하네스에도 반드시 넣을 것. 쿼리는 **기본이 부분일치**다(`exact: true`를 안 주면 "금액" 섹션 제목이 "채권발행금액" 같은 라벨에도 매치하는 식으로 모호해진다 — 실제로 여러 번 겪음). `window.location`은 실제 브라우저에서 `location`(Window 소유)·`href`(Location 소유) 둘 다 own accessor property이자 `configurable:false`라 jsdom과 달리 `defineProperty`로 가로챌 방법이 없다(실측 확인, 세 브라우저 전부) — 클릭 시 실제 네비게이션이 일어나는지는 컴포넌트 테스트가 아니라 Playwright E2E로 검증할 것. Browser Mode용 `pnpm test`/`pnpm test --project=browser`는 이 세션(Claude Code)에서 `pnpm exec vitest ...`가 아니라 반드시 `pnpm test ...` 형태로 호출할 것 — `.claude/settings.json`의 `sandbox.excludedCommands`가 `"pnpm test *"` 문자열 그대로만 매칭해 샌드박스를 벗어나는데, `pnpm exec vitest`는 매칭되지 않아 샌드박스 안에서 크로미움이 `bootstrap_check_in ... Permission denied`로 죽는다(실측 확인). 또한 `pnpm test -- --project=browser`처럼 pnpm 자체의 `--`를 끼워 넣으면 그 `--`가 vitest에 그대로 전달돼 옵션 파싱이 깨진다(전체 프로젝트가 도는 것처럼 보임) — `pnpm test --project=browser --run`처럼 `--` 없이 직접 붙일 것.
  - **Playwright E2E(`e2e/`, `playwright.config.ts`)**: `pnpm test:e2e`(스크립트) 또는(이 세션에서는 샌드박스 예외 매칭을 위해) `pnpm exec playwright test ...`로 실행한다. `webServer`가 `pnpm dev`를 띄우고, 각 스펙이 `page.route()`로 `/api/snapshot/*`를 모킹한다(`e2e/fixtures/snapshot.ts`, `src/lib/snapshot/encode.ts`를 상대 경로로 직접 import해 픽스처를 만든다 — 이 파일이 `@/` 별칭을 안 쓰는 이유는 위 "데이터 계층" 절 참고) — `pnpm seed:local`(`.backfill/` 319MB, 오픈API 재수집 필요)이 필요 없다. 상세 페이지(`/bond/[id]`)는 SSR이 D1을 직접 타 이 모킹 대상이 아니다 — E2E에서는 라우팅 전이(URL이 바뀌는 것)까지만 검증하고 응답 내용은 단언하지 않는다. `screener.spec.ts`류(데스크톱 표 레이아웃, `<tr>` 1개/종목, 고정 td 인덱스 가정)는 `test.skip(({ isMobile }) => isMobile, ...)`로 `Mobile Chrome` 프로젝트를 건너뛴다 — 모바일(종목당 2행)은 `responsive.spec.ts`가 반대로 `test.skip(({ isMobile }) => !isMobile, ...)`로 데스크톱을 건너뛰며 따로 검증한다. `ScreenerFilterRange` 트리거(예: "만기일", "표면이율")는 접근성 이름이 같은 이름의 표 헤더 정렬 버튼과 충돌한다(다중선택 트리거는 "라벨 전체/개수" 접미사가 붙어 안 겹침) — `exact: true`와 DOM 순서(필터 바가 표보다 먼저 나옴, `.first()`)로 구분할 것. `useScreenerData`가 TanStack Query의 기본 retry(3회, 지수 백오프)를 끄지 않으므로, fetch 실패 → 에러 화면 전환을 기다리는 단언은 기본 5초보다 넉넉한 타임아웃(예: 15초)을 줄 것 — 안 그러면 재시도 도중(`isPending` 유지) 타임아웃돼 "0건"으로 보이는 중간 상태를 에러로 오인해 실패한다.
  - **`astro dev`가 이 세션(Claude Code)에서 자동으로 백그라운드 데몬이 된다 — Playwright의 `webServer`를 깨뜨린다.** Astro 7.2({astro}/dist/cli/agent.js)는 `am-i-vibing` 패키지로 "AI 에이전트가 실행 중인지"를 `CLAUDECODE` 등 환경변수로 감지하고, 감지되면 `--background` 플래그 없이도 dev 서버를 백그라운드 데몬으로 띄운 뒤 launcher 프로세스를 즉시 종료한다(실측 확인: 플래그 없이 `astro dev`를 실행해도 상태 메시지 한 줄만 찍고 바로 끝나며, 실제 서버는 `astro dev status`로 확인되는 별도 프로세스로 계속 돈다). Playwright는 `webServer.command`가 계속 살아있는 포그라운드 프로세스라고 가정하므로, 이 자동 전환을 `Process from config.webServer exited early`로 오인해 실패한다. `playwright.config.ts`의 `webServer.env`에 `{ CLAUDECODE: "" }`를 넘겨 감지를 끈다(`am-i-vibing`의 `checkEnvVar`가 `Boolean(value)` 판정이라 빈 문자열은 falsy로 처리됨 — 실측 확인). 이 프로젝트 밖에서 `astro dev`를 다른 자동화(cron, CI 스크립트 등)로 띄울 때도 같은 문제가 재현될 수 있다.
...
- **스크리너 스냅샷 — base(주 1회) + bond 델타(매 영업일)**: 기본정보가 매 영업일 수집되지만(위 "cron 파이프라인" 절), base(`snapshot/bond/{basDt}.json`, 3MB대, `immutable` 캐시)를 매일 재빌드하면 방문자가 매일 그 전체를 다시 받는다. 그래서 base는 `SNAPSHOT_REBUILD_WEEKDAY_KST`(기본 수요일, `src/lib/sync/config.ts`)에만 전량 재빌드(compaction)하고, 그 외 평일에는 그날 변경된 종목만 담은 작은 **bond 델타**(`snapshot/bond-delta/{basDt}.json`)를 올린다 — 시세가 이미 쓰던 base+delta 패턴(아래 "읽기 계층 — 목록" 절)을 bond 축으로 확장한 것이다. `src/lib/sync/plan.ts`의 분기: `issu`가 오늘 basDt로 `done`이고 base가 그 basDt로 아직 없으면(재빌드 요일이거나 base 자체가 없으면) `snapshot`, 아니면 `bondDelta`. base 재빌드가 `SNAPSHOT_MAX_ATTEMPTS`를 소진해 포기해도 `bondDelta`로 자동 폴백해 최신을 유지한다.
  ...
  **basDt 정체성 규칙(중요)**: base의 R2 키·`index.json`·`app_meta.snapshot_bas_dt`는 항상 "수집 대상 basDt"(그 tick의 `issu` run이 조회한 basDt)를 쓴다 — `bond.last_chg_bas_dt`의 최댓값(그날 bond 정적 필드가 실제로 바뀐 날짜)을 쓰지 않는다. `src/lib/snapshot/encode.ts`의 `createSnapshotBuilder().finish(basDtOverride)`가 이 override를 받고, `buildAndPutSnapshot(env, targetBasDt)`가 항상 넘긴다. 이유: 그날 bond 정적 필드가 하나도 안 바뀐 날(흔하다 — `bond_state`만, 즉 신용등급·잔액만 바뀐 날도 있다)에 `MAX(last_chg_bas_dt)`를 쓰면 (a) `planTick`이 매 tick마다 "아직 이 basDt로 스냅샷 안 만듦"으로 오판해 cron 창 내내 재빌드하거나, (b) 그 전날과 같은 R2 키에 다른 내용을 쓰게 되는데 그 키는 `immutable` 캐시라 클라이언트가 새 내용을 영영 못 받는다 — 둘 다 실제로 겪을 뻔한 버그다. `scripts/build-snapshot.mjs`(수동 폴백)는 "수집 대상"이라는 개념이 없어 override 없이 기존 `MAX(last_chg_bas_dt)` 동작을 유지한다.

  base 빌드는 `bond`(정적 필드) + `bond_state`(현재값, `LEFT JOIN`) + `code_label` + 종목별 "최신" 시세(`bond_price`를 `MAX(bas_dt)` 서브쿼리로 조인) 4종을 `src/lib/d1/snapshot-repo.ts`가 **키셋 페이지네이션**(`isin_cd` 오름차순 청크, `SNAPSHOT_CHUNK_SIZE=5000`)으로 읽어 `src/lib/snapshot/encode.ts`의 `createSnapshotBuilder()`에 순차로 흘려 넣는다 — 위 "2026-09-01부터" 단락에서 설명한 128MB isolate 메모리 제약 때문에 전량 `all()`을 쓰지 않는 것이 핵심 설계 포인트다. `bond_price` 서브쿼리 페이지가 한 `isin_cd`에 KTS·일반채권 두 행을 낼 수 있어(0001_init.sql의 `bond_price` PK 주석), 페이지 종료 판단은 "받은 행 수"가 아니라 "페이지 내 **고유** `isin_cd` 수"로 한다(`buildAndPutSnapshot`의 `distinctIsinCds` 참고) — 안 그러면 그 종목 바로 다음 종목을 건너뛸 수 있다.

  **bond 델타**(`src/lib/snapshot/bond-delta.ts`, `buildAndPutBondDelta`)는 그날의 변경분을 D1 재조회로 얻는다(`SNAPSHOT_BOND_CHANGED_SQL`: `WHERE b.last_chg_bas_dt = ?1 OR s.valid_from = ?1`) — `writeBondPage`가 페이지마다 들고 있는 변경분을 tick 경계 너머로 누적할 방법이 없고, 한 basDt 수집이 여러 tick에 걸칠 수 있기 때문이다. 이 조건이 "bond 정적 필드 변경(신규 종목 포함) OR bond_state 변경(신용등급·잔액)"을 정확히 덮는다. 델타 한 행은 시세 델타(변경된 필드만)와 달리 그 종목의 **그날 현재 전체 상태**를 담는다 — 그래서 여러 날짜의 델타가 쌓여도 basDt 오름차순으로 그냥 덮어쓰면(`src/lib/snapshot/merge.ts`의 `mergeBondDeltas`) 항상 최신이 남는다. `bond_isur_nm`은 base처럼 사전 인덱스가 아니라 원문 문자열로 싣는다(델타가 base의 `issuers` 사전을 모르므로) — 병합 시 그 사전에 인턴된다. 변경 행이 `BOND_DELTA_MAX_ROWS`(5000)를 넘으면 델타 대신 전량 재빌드로 폴백한다(`tooLarge`, `src/lib/sync/tick.ts`의 `runBondDeltaAction`) — 그 지점부터는 델타가 base에 근접해 base+delta의 이점이 사라지기 때문이다.
