...
- **`search_bonds`의 `kind`(채권 종류)는 코드가 아니라 한글 라벨을 받는다.** `bond.scrs_itms_kcd` 코드(1101 국채 등)를 LLM이 알 수 없어서인데, 코드↔라벨 맵을 소스에 이중 관리하지 않으려고 SQL의 `code_label` 서브쿼리로 해석한다 — `src/lib/bond/columns.ts`의 `BOND_KIND_LABELS`는 zod `z.enum` 선택지 노출용 사본일 뿐 정본이 아니다(정본은 D1 `code_label(domain='scrsItmsKcd')`). 신용등급 하한(`minGrade`)은 `src/lib/bond/grade.ts`의 `gradesAtOrAbove`로 `IN (...)` 목록으로 펼친다 — SQL에서 문자열 등급을 부등호로 비교할 수 없어서다. 이 파일은 원래 `src/lib/screener/format.ts`에 있던 `GRADE_ORDER`/`compareGrade`를 Worker와 공유하려고 승격한 것이라, `format.ts`가 `compareGrade`를 재export해 클라이언트 import 경로를 유지한다.
