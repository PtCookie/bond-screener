...
export function ScreenerPresetMenu({ presets, currentQuery, onSave, onDelete, onApply }: ScreenerPresetMenuProps) {
  ...
  function commit() {
    onSave(trimmedName, currentQuery);
    reset();
  }
      ...
      <PopoverContent align="start" className="w-72 gap-3">
        {pendingOverwrite === null ? (
          ...
          <div className="flex flex-col gap-2">
            <p className="text-muted-foreground">“{pendingOverwrite.name}” 프리셋을 덮어쓸까요?</p>
            ...
            {presets.map((preset) => (
              <div key={preset.id} className="hover:bg-muted flex items-center gap-1 rounded-lg pr-1">
                ...
                <Button
                  ...
                  onClick={() => {
                    onDelete(preset.id);
                    if (pendingOverwrite?.id === preset.id) setPendingOverwrite(null);
                  }}
                >
                  <TrashIcon />
                </Button>
